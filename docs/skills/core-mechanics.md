...
**Surprise/passive vs. active: ability-only save vs. full skill check.** When a character is caught
off guard by something they weren't actively engaging with, they roll a save using only their raw
ability modifier -- no skill rank. When a character is actively, deliberately responding to the same
kind of challenge, they roll the full skill check (ability modifier + skill rank) instead. Both
target the same DC, generated by whatever's being resisted -- the DC doesn't change based on whether
the defender happens to be caught by surprise or acting deliberately, only whether their own roll
gets to add a skill rank on top of the ability modifier. This is a general principle, not limited to
Perception (though it resolves Perception specifically now that Perception is an ability score, not a
skill -- see `docs/skills/skill-list.md`'s Spot/Search entries for the active side).

> **Example (Str):** entangling plants spring up around a character. A natural, unfocused attempt to
> avoid being caught is a **Str save** (ability only) at the DC. If the save fails and the character
> is now entangled, a later **deliberate attempt to break free** is a full **Athletics check**
> (ability + rank) against the same DC.

This means skill investment matters for defense as much as for offense -- a trained response beats an
instinctive one at the same target number, consistently, on both sides of any given interaction.
