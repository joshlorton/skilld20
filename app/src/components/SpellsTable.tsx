...
interface Props {
  columns: ColumnDef<SpellEntry>[];
  rows: SpellEntry[];
  editable?: boolean;
  onCellCommit?: (index: number, patch: Partial<SpellEntry>) => void;
...
function cellText(row: SpellEntry, col: ColumnDef<SpellEntry>): string {
...
export function SpellsTable({
  ...
  const [sortKey, setSortKey] = useState<(keyof SpellEntry & string) | null>(null);
  ...
  function toggleSort(key: keyof SpellEntry & string) {
    if (sortKey === key) {
            ...
            <div key={col.key} className={col.cls}>
              {col.key === 'name' ? (
                editable ? (
                  <>
                    <EditableField
                      ...
                      value={row.name}
                      onCommit={(v) => onCellCommit?.(index, { name: v })}
                    />
                    <EditableArrayField
                      className="mat-name-aka-input"
                      value={row.nicknames}
                      onCommit={(v) => onCellCommit?.(index, { nicknames: v })}
                    />
                  </>
                ) : (
                  <>
                ...
                editable ? (
                  <EditableArrayField
                    value={row.traits}
                    onCommit={(v) => onCellCommit?.(index, { traits: v })}
                  />
                ) : (
                  row.traits.join(', ')
                )
                ...
                <EditableField
                  ...
                  onCommit={(v) => onCellCommit?.(index, { [col.key]: v } as Partial<SpellEntry>)}
                />
              ) : (
                cellText(row, col)
              )}
